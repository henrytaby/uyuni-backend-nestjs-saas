...
tenant-isolation semantics where applicable. Note: `User` is _global_
tenant link.

**Migration Note**: Este spec persiste 4 modelos (`Plan`, `Tenant`, `User`, `TenantUser`). El `schema.prisma` actual en el repo contiene también los modelos de specs 003+ (`RefreshToken`, `Role`, etc.) ya implementados — no son parte de THIS spec.
...
| created_by_id | UUID     | FK → User, nullable           | Auto-injected (bridge in this spec; full auditColumnsExtension lives in spec 005; spec 002 ships the tenant-scoped extension as the bridge; cdcExtension/appendOnlyExtension land con 005)                |
| updated_by_id | UUID     | FK → User, nullable           | Auto-injected                                                                                                                                                                                             |
| deleted_by_id | UUID     | FK → User, nullable           | Auto-injected on soft-delete (bridge in this spec; full auditColumnsExtension lives in spec 005; spec 002 ships the tenant-scoped extension as the bridge; cdcExtension/appendOnlyExtension land con 005) |
...
- module_access: must be a JSON array of strings from the canonical set
  ...
  (Note: validated in DTO, DB just stores `JsonB` without array constraints).
...
tenant root). RLS applies to all _other_ tenant-scoped tables via FK to
...
**Indexes**: `@unique([slug])` (Note: Implemented as a partial unique index `WHERE is_active = true` via custom migration), `(plan_id)`.
...
| deleted_by_id         | UUID     | FK → User (self), nullable    | Auto-injected on soft-delete (bridge in this spec; full auditColumnsExtension lives in spec 005; spec 002 ships the tenant-scoped extension as the bridge; cdcExtension/appendOnlyExtension land con 005) |
...
need a separate schema migration. The _verification token / OTP_ storage
...
**Back-Relations Note:** Back-relations to Role, Permission, RoleAssignment, RefreshToken, AccessLog, ChangeRecord exist in `schema.prisma` from this migration but are owned by specs 003/004/005 — this spec only persists the User entity itself.
...
| User       | ❌ No  | Global identity    | App guards restrict (own record / members visible). _Note: Anti-leakage tests currently cover `TenantUser` (RLS-enforced) and `Tenant` GET cross-tenant. Coverage for `User` endpoints depends solely on `PlatformAdminGuard` (since `User` is not tenant-scoped) — this is a known limitation tracked for hardening._ |
...
All _future_ tenant-scoped domain entities (CRM, Sales, etc.) will carry
...
| Tenant     | (slug)               | Partial Unique | URL routing (`WHERE is_active = true`)           |
| Tenant     | (plan_id)            | B-tree         | Plan lookups                                     |
| User       | (email)              | Partial Unique | Global login identity (`WHERE is_active = true`) |
...
interface TenantContext {
  ...
  scopeFilter?: 'ANY' | 'OWN'; // Forward-compatible with RBAC ownership scope (spec 004). In spec 002 it remains undefined (the OWN branch is inert). OwnershipScopeInterceptor populates it from effectivePermissions once RBAC is loaded.
}
```
...
| created_by_id | UUID FK→User | Prisma extension reading TenantContext **— bridge in this spec; full auditColumnsExtension lives in spec 005; spec 002 ships the tenant-scoped extension as the bridge; cdcExtension/appendOnlyExtension land con 005** |
| updated_by_id | UUID FK→User | Prisma extension reading TenantContext **— bridge in this spec; full auditColumnsExtension lives in spec 005; spec 002 ships the tenant-scoped extension as the bridge; cdcExtension/appendOnlyExtension land con 005** |
| deleted_by_id | UUID FK→User | Prisma extension reading TenantContext **— bridge in this spec; full auditColumnsExtension lives in spec 005; spec 002 ships the tenant-scoped extension as the bridge; cdcExtension/appendOnlyExtension land con 005** |
