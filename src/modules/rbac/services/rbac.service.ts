...
export class RbacService {
  ...
  async createRole(dto: CreateRoleDto, tenantId: string, userId: string) {
    ...
    if (existing) throw new BadRequestException('Role already exists in this tenant');
  ...
  async updateRole(id: string, dto: UpdateRoleDto, tenantId: string, userId: string) {
    ...
    if (!role) {
      ...
      if (systemRole) throw new ForbiddenException('Cannot modify system roles');
  ...
  async assignRole(dto: RoleAssignmentDto, tenantId: string, userId: string) {
    ...
    if (!role) throw new NotFoundException('Role not found or not available in this tenant');

    const tenantUser = await this.prisma.tenantUser.findFirst({
      where: { id: dto.tenantUserId, tenantId }
    });

    if (!tenantUser) throw new NotFoundException('TenantUser not found in this tenant');
